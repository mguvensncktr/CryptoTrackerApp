...
export const allPortfolioBoughtAssetsFromAPI = selector({
    ...
    get: async ({ get }) => {
        ...
        const portfolioAssetMarketData =
            await getWatchListedCoins(boughtPortfolioAssets.map((portfolioAsset) => portfolioAsset.id).join(','))

        const boughtAssets = boughtPortfolioAssets.map((boughtAsset) => {
            const portfolioAsset = portfolioAssetMarketData.filter((item) => boughtAsset.id === item.id)[0]
            return {
                ...boughtAsset,
